...
function renderHold(){
  ...
  groups.forEach(g=>{
    ...
    g.items.forEach(h=>{
      ...
      rows+='<tr><td><div class="name-cell"><span class="nm">'+nameOf(h.code)+'</span><span class="cd">'+h.code+'</span></div></td>'
        ...
        +'<td><div class="addcash-cell"><input class="ain reducash" type="number" min="0" step="0.01" data-rc="cash" data-code="'+h.code+'" placeholder="金额" style="width:80px;"><button class="ain-btn red" data-hred="'+h.code+'" title="按现价一键减仓（需先有行情/净值）">减仓</button></div></td>'
  ...
  box.querySelectorAll('button[data-hred]').forEach(b=>{ b.onclick=()=>{
    const code=b.dataset.hred; const inp=box.querySelector('input[data-rc="cash"][data-code="'+code+'"]');
    const cash = inp ? parseFloat(inp.value) : 0;
    reducePositionCash(code, cash);
  }; });
}
...
function reducePositionCash(code, cash){
  ...
  renderHold(); renderHoldSelect();
  if(code===state.selected){ const wk=(state.watch.find(x=>x.code===code)||{}); if(wk.kind!=='fund') renderDetail(); }
  const rp=r.realizedPnl, rpTxt=(rp>=0?'实现盈利 +':'实现亏损 ')+fmt(Math.abs(rp),2)+' 元';
  const clearTxt = (r.newShares<=0)?'（已清仓）':'（余 '+fmt(r.newShares,isF?2:0)+' 股，成本价不变 '+fmt(r.newCost,isF?4:2)+'）';
  toast('✓ 减仓成功：现价 '+fmt(r.price, isF?4:2)+' 卖出 '+fmt(r.sellShares, isF?2:0)+' 股，'+rpTxt+clearTxt);
}
