/* 阶段5e：样本外时间验证 —— 固定最优条件，按2025-01-01切分，看近期段(样本外)是否仍>80%
 * 目的：检验"估值<5%+RSI<22+周跌>3%→持有250日81%"是真规律还是特定熊市样本过拟合。
 */
...
async function main(){
  ...
  for(const code in all){ for(const r of all[code].recs){
    const dc=r.dayClose, L=r.L, di=r.dayIdx; const t=di+250; if(t>=L){continue;}
    let dirty=false; for(let k=di+1;k<=t;k++){ if(Math.abs(dc[k]/dc[k-1]-1)>0.25){dirty=true;break;} }
    if(dirty) continue;
    rows.push({date:r.date, ret:(dc[t]/dc[di]-1)*100});
  } }
  function seg(name, pred){ const v=rows.filter(pred); if(!v.length){ console.log('  '+name+': n=0'); return; }
    const up=v.filter(r=>r.ret>0).length, m=v.reduce((s,r)=>s+r.ret,0)/v.length;
    console.log('  '+name+': n='+v.length+'  250日绝对上涨命中='+(up/v.length*100).toFixed(0)+'%  均值='+(m>=0?'+':'')+m.toFixed(0)+'%'); }
  console.log('最优条件信号样本 n='+rows.length+' (N=250日)');
  seg('全样本(2022-2026)', ()=>true);
  seg('训练段(<2025-01-01)', r=>r.date<'2025-01-01');
  seg('样本外段(>=2025-01-01)', r=>r.date>='2025-01-01');
}
